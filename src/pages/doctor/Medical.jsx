...
import { Loader2, Activity, Search, DollarSign, Trash2, Edit3, AlertTriangle } from "lucide-react";
...
export default function Medical() {
  ...
  // 3. Handle Add/Update Stock (From Pharmacy Side)
      ...
      <Card className="rounded-[2rem] shadow-xl border-none overflow-hidden ring-1 ring-slate-100">
        <div className={`${editId ? 'bg-amber-500' : 'bg-blue-600'} px-8 py-3 flex justify-between items-center`}>
            <span className="text-white text-[10px] font-black uppercase tracking-widest">
              {editId ? 'Modify Medicine Data' : 'Add Stock to Inventory'}
            </span>
            {editId && (
        ...
        <CardContent className="p-8 grid md:grid-cols-5 gap-4 items-end">
          ...
          <div className="space-y-1">
            <span className="text-[10px] font-bold uppercase text-slate-400 ml-1">Unit Price ($)</span>
          ...
          <div className="space-y-1">
            ...
            <select 
              ...
              className="w-full h-11 rounded-xl bg-slate-50 border-none px-3 text-sm font-bold uppercase"
            >
        ...
        <Table>
          <TableHeader className="bg-slate-50">
            <TableRow className="border-none">
              ...
              <TableCell className="font-bold uppercase text-[11px] text-slate-400">Inventory Value</TableCell>
              <TableCell className="font-bold uppercase text-[11px] text-slate-400">Current Stock</TableCell>
              ...
              <TableRow key={item.id} className={`hover:bg-slate-50/50 border-slate-50 ${item.quantity <= 0 ? 'bg-red-50/30' : ''}`}>
                <TableCell className="py-4 pl-8 font-black uppercase text-slate-700">
                  <div className="flex flex-col">
                    {item.medicineName}
                    {item.quantity <= 0 && <span className="text-[9px] text-red-500 flex items-center gap-1"><AlertTriangle size={10}/> OUT OF STOCK</span>}
                  </div>
                </TableCell>
