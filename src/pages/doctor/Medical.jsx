...
import { Loader2, PackagePlus, Store, Activity, Search, ChevronLeft, ChevronRight, AlertCircle } from "lucide-react";
...
export default function Medical() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (!existSnap.empty) {
        // Update existing: We add to the 'quantity' field
        ...
        await addDoc(collection(db, "branch_medicines"), {
          ...
          stockQuantity: 0, // This is where subtractions live
  ...
  if (loading && !userData) return <div className="h-screen flex items-center justify-center"><Loader2 className="animate-spin" /></div>;
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <Card className="rounded-[2rem] shadow-xl border-none">
        <CardContent className="p-8 grid md:grid-cols-4 gap-4 items-end">
          ...
          <Button onClick={handleAdd} className="h-11 rounded-xl bg-blue-600 font-bold uppercase text-xs tracking-widest">Update Stock</Button>
        </CardContent>
      </Card>
      ...
      <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
        <Table>
          <TableHeader className="bg-slate-50">
            <TableRow>
              <TableCell className="font-bold py-4 pl-6">Medicine</TableCell>
              <TableCell className="font-bold">Branch</TableCell>
              <TableCell className="font-bold text-right pr-6">Current Balance</TableCell>
            </TableRow>
          </TableHeader>
          <TableBody>
            {currentItems.map((item) => {
              // --- THE MATH LOGIC ---
              const initial = Number(item.quantity || 0);
              const used = Number(item.stockQuantity || 0);
              const balance = initial + used; // e.g. 12 + (-8) = 4
              const isLow = balance < 5;

              return (
                <TableRow key={item.id} className="hover:bg-slate-50">
                  <TableCell className="py-4 pl-6 font-bold uppercase text-slate-700">{item.medicineName}</TableCell>
                  <TableCell className="text-xs font-medium text-slate-500">{item.branchId}</TableCell>
                  <TableCell className="text-right pr-6">
                    <Badge className={`rounded-lg px-3 py-1 font-black ${isLow ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'}`}>
                      {balance} Units
                    </Badge>
                  </TableCell>
                </TableRow>
