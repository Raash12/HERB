...
import { LayoutDashboard, UserPlus, LogOut, Moon, Sun, Users, Search, FileText, Activity, Zap, ShieldCheck, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function ReceptionDashboard() {
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      ...
      // 2. Listen for Optical Prescriptions (Independent)
      ...
      // 3. Listen for Medical Prescriptions (Independent)
      ...
      return () => { unsubPatients(); unsubOptical(); unsubMedical(); };
    };

    setupListeners();
  }, []);

  // Isku dar labada xogood mar kasta oo midkood isbeddelo
  useEffect(() => {
                ...
                <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-xl">
                  <div className="relative w-96">
                    ...
                    <Input 
                      ...
                      className="pl-14 h-14 border-none bg-slate-50 dark:bg-slate-800 rounded-2xl" 
                      value={searchTerm} 
                      onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} 
                    />
                  </div>
                  <Badge className="bg-blue-600 px-6 py-2 rounded-xl uppercase text-[10px] font-black">{filteredData.length} Found</Badge>
                </div>
                ...
                <Card className="rounded-[3rem] border-none shadow-2xl bg-white dark:bg-slate-900 overflow-hidden">
                  ...
                  <div className="p-8 flex items-center justify-between border-t dark:border-slate-800">
                    ...
                    <div className="flex gap-3">
                      <Button variant="outline" className="h-10 w-10 rounded-xl" disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)}><ChevronLeft size={18}/></Button>
                      <Button variant="outline" className="h-10 w-10 rounded-xl" disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)}><ChevronRight size={18}/></Button>
                    </div>
                  </div>
                </Card>
              </div>
...
function NavItem({ label, icon, active, onClick }) {
  return (
    <SidebarMenuItem>
      <SidebarMenuButton onClick={onClick} className={`w-full flex items-center gap-4 p-4 h-14 rounded-2xl transition-all ${active ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-white/5'}`}>
...
function MiniStat({ label, value, icon, color }) {
  const colors = { blue: "text-blue-600 bg-blue-50", emerald: "text-emerald-600 bg-emerald-50", indigo: "text-indigo-600 bg-indigo-50", rose: "text-rose-600 bg-rose-50" };
  return (
    <Card className="border-none shadow-xl p-8 rounded-[3rem] bg-white dark:bg-slate-900">
